...
export default function ProfessionalAddRecordModal({ isOpen, onClose, onRecordAdded, user, patientPHN }) {
  ...
  useEffect(() => {
    if (isOpen) {
      // Pre-fill the healthcare professional's data
      setFormData(prev => ({
        ...prev,
        recordId: generateRecordId(),
        slmcNo: user?.userIdentifier || '', // Auto-fill SLMC
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const recordData = {
        ...formData,
        personalHealthNo: patientPHN,
        slmcNo: user.userIdentifier,
      };

      console.log('Submitting record:', recordData);

      const response = await api.post('/api/professional/medical-records', recordData);
      ...
      <div className="bg-white rounded-lg p-6 w-full max-w-5xl my-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="flex justify-between items-center">
            <h2 className="text-2xl font-bold">Add Medical Record for PHN: {patientPHN}</h2>
